...
{	
	...
	Arena *currentStateArena = gameState->currentStateArena;
	Font font = gameState->font;
	ExecuteData *exe = &gameState->executeData;
	...
	// HandleInput
	{
		...
		for (u32 i = 0; i < buffer.amountOfMessages; i++)
		{
			...
			{
				viewLighting = !viewLighting;
				//CalculateLightingSolution(level, gameState->constantArena);
			}
		}
	}
	...
	case Game_Editor:
	{
		RenderEditor(rg, assetHandler, gameState->editor, world, input);
	}break;
	case Game_Execute:
	{
	...
	PushString(rg, V2(0.75f, 0.01f), GameModeStrings[gameState->mode], 0.03f, font);
