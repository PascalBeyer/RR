...
SteeringList *GetDistNeighborhood(UnitSteeringData *unit, UnitSteeringData *selection, u32 amount, float desiredDist)
{
...
SteeringList *GetNeighborhood(UnitSteeringData *unit, UnitSteeringData *selection, u32 amount)
{
//	SteeringList *ret;
...
void ApplyCollisionSteering(UnitSteeringData *from, UnitSteeringData *to)
{
	...
	//if (quadRadSum > quadDist)
	{
		...
		if (quadDist != 0)
		{
			...
			float p = Dot(relPos, relPos + relVel * dt) / qNormRPos;
			float q = (QuadNorm(relPos + relVel * dt) - radSum) / qNormRPos;
			float disc = p * p - q;
		...
		if ((from->idle ^ to->idle))
		{
			...
			if (!from->idle)
			{
				to->steering += relPos * (combinedSteeringForce / dt);
			}

			if (!to->idle)
			{
				from->steering -= relPos * (combinedSteeringForce / dt);
			}
		}
		else
		{
			to->steering += relPos * (combinedSteeringForce / (2 * dt));
			from->steering -= relPos * (combinedSteeringForce / (2 * dt));

		}
	}
}

void ApplyGoalSeekSteering(UnitSteeringData *unit)
{
	v2 goal = unit->goal;
	v2 pos = unit->pos;
...
void ApplyArrivalAndSeekSteering(UnitSteeringData *unit)
{
	v2 pos = unit->pos;
	v2 goal = unit->goal;
...
void ApplyAlignmentSteering(UnitSteeringData* unit, SteeringList *neighbors, float weight)
{
	v2 steer = V2();
	u32 count = 0;
...
void ApplyCohesionSteering(UnitSteeringData *unit, SteeringList *neighbors, float weight)
{
	v2 avgPos = V2();
	u32 count = 0;
...
void ApplySeparationSteering(UnitSteeringData *unit, SteeringList *neighbors, float weight)
{
	for (SteeringList *it = neighbors; it; it = it->next)
	{
...
void ApplyCollisionSeparationSteeringNSquared(UnitSteeringData *selection, u32 amount)
{
	for (u32 i = 0; i < amount; i++)
	{
...
u32 SteeringListCount(SteeringList *list)
{
	if (!list) return 0;
	return (SteeringListCount(list->next) + 1);
}

void TileMapCollisionMovement(v2 *pos, float radius, v2 by, v2 currentGoal, TileMap *tileMap)
{
	v2 unitPos = *pos;	
	float xMin;
	float xMax;
...
void MoveSelection(UnitSelection *selection, UnitSteeringData *data, TileMap *tileMap)
{
...
void CollectSteeringData(UnitSelection *selection, UnitSteeringData *data)
{
	if (selection->amountSelected > 0)
	{
		dt = selection->Get(0)->expectedSecondsPerFrame;
	}
...
Tile *FirstPathBlockate(TileMap tileMap, v2 pos,  v2 goal)
{
	return GetFirstTileWithType(tileMap, pos, goal, 1);
}

bool SolveLineIntersectoion(float *u, float *v, v2 a, v2 b, v2 c, v2 d)
{
	v2 s = d - b;
...
void SimpleStupidFunnelAlgorithm(Unit *unit, TileMap tileMap)
{
...
void ClearSteeringList(SteeringList *list)
{
	if (list)
	{
...
void MoveUnits(UnitSelection *selection, TileMap tileMap)
{
