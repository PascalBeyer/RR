...
//Globals
const u32 imageSizeFactor = 80; // 80 = 1280 by 720 (actual size)
const u32 imageWidth = imageSizeFactor * 16;
const u32 imageHeight = imageSizeFactor * 9;
Bitmap bitmap;
u32 image[imageWidth][imageHeight];
AssetHandler *assetHandl;
World globalWorld;
...
static void InsertSample(KdNode *node, IrradianceSample *sample)
{
	...
	InsertSample(node->negative, sample);
	InsertSample(node->positive, sample);
}
...
static TriangleSearchResultWide GetInitialTriangleWide(v3Wide rayEntry, v3Wide xyD, KdNode *entryLeaf)
{
	...
	{		
		...
		while (AnyTrue(AndNot(iMask, finishedMask)))
		{
			...
			{								
				t[ti] = currentLeaf[ti]->triangles[Min(i, currentLeaf[ti]->amountOfTriangles - 1)];				
			}			
			...
			f32Wide lenD1Sq = LoadFromStruct(t, lenD1Sq);
			f32Wide lenD2Sq = LoadFromStruct(t, lenD2Sq);
			f32Wide d12 = LoadFromStruct(t, d12);
			f32Wide denomInv = LoadFromStruct(t, denomInv);
...
IrradianceSample SampleIrradiance(v3 initialP, LightingTriangle *initialT, KdNode *initialLeaf, v3 lightSource, v3 initialAttenuation, u32 rayAmountPerBounceAmount, u32 maxBounceCount, RandomSeries *entropy, AABB sceneAABB)
{
	f32 epsilon = 0.001f;
	...
	ret.triangleIndex = initialT->index;

	return ret;
}

//todo: if its _really_ fast now, we may functionalize it?
RayCastResult CastRaysCache(Bitmap bitmap, ClipRect clipRect, World world)
{	
	...
	v3 cameraPlaneUL = camP + camB3 - 0.5f * aspectRatio * camB1 + 0.5f * camB2;
	...
	for (u32 h = clipRect.yMin; h < clipRect.yMax; h++)
	{
		...
		for (u32 w = clipRect.xMin; w < clipRect.xMax; w++)
		{
			...
			v3 xOff = (aspectRatio * (xAdj / (f32)imageWidth)) * camB1;
			v3 yOff = (yAdj / (f32)imageHeight) * camB2;

			v3 rayDUN = cameraPlaneUL + xOff - yOff - camP;
			v3 xyD = FastNormalize(rayDUN); //biased?
			
			//Interval rayIntersectionInverval = GetIntersectionRayAABB(camP, xyD, kdTree->aabb);
			//if (rayIntersectionInverval == InvertedInfinityInterval())
			//{				
			//	//ray does not pas though the scene (do nothing) 
			//	*xIt++ = 0xffc0cb;
			//	continue;				
			//}
			//
			//v3 rayEntry = xyD * rayIntersectionInverval.min + camP;
			u64 timer = __rdtsc();
			...
			u32 entriesFound = 0;
			for (u32 cacheIt = 0; cacheIt < initialT->amount; cacheIt++)
			{
				IrradianceSample *entry = initialT->entries[cacheIt];				
			...
			IrradianceSample *newEntry = PushStruct(world.cache->irradianceArena, IrradianceSample);
			*newEntry = SampleIrradiance(initialP, initialT, initialLeaf, lightSource, initialAttenuation, rayAmountPerBounceAmount, maxBounceCount, &entropy, kdTree->aabb);
			InsertSample(kdTree, newEntry);

			*xIt++ = (Pack3x8(indirectColor));
		}
	}
...
RayCastResult CastRaysCacheWide(Bitmap bitmap, ClipRect clipRect, World world)
{
	u32 rayAmountPerBounceAmount = 1024u;
	u32 maxBounceCount = 2u;
	...
	v3Wide cameraPlaneUL = camP + camB3 - 0.5f * aspectRatio * camB1 + 0.5f * camB2;
	...
	{		
		for (u32 w = clipRect.xMin; w < clipRect.xMax; w += 2)
		{
			...
			v3Wide xOff = (aspectRatio * (xAdj / (f32)imageWidth)) * camB1;
			v3Wide yOff = (yAdj / (f32)imageHeight) * camB2;
			...
			*GetPixel(bitmap, w + 1, h + 1) = Pack3x8(firstTResult.initialT[3]->color);
			...
			v3Wide initialN = Load(initialT[0]->normal, initialT[1]->normal, initialT[2]->normal, initialT[3]->normal);
			v3Wide initialAttenuation = Max(0.0f, Dot(-xyD, initialN)) * Load(initialT[0]->color, initialT[1]->color, initialT[2]->color, initialT[3]->color);
			...
			for (u32 laneIndex = 0; laneIndex < 4; laneIndex++)
			{
				...
				for (u32 cacheIt = 0; cacheIt < initialT[laneIndex]->amount; cacheIt++)
				{
					IrradianceSample *entry = initialT[laneIndex]->entries[cacheIt];
...
void LightingMain(RenderCommands *renderComands, WorkHandler *workHandler, Input *input)
{
	...
	if (debugViewSwitch)
	{
		...
		PushOrthogonalTransform(&rg, (f32)imageWidth, (f32)imageHeight);
		PushClear(&rg, clearColor);
	...
	{
		PushProjectivTransform(&rg, globalWorld.camera.pos, globalWorld.camera.basis, 1280.0f/720.0f, 1.0f);
		...
		{
			PushTriangle(&rg, V3(), V3(1, 0, 0), V3(1, 1, 0), V4(1, 1, 1, 0));
			rg.currentTriangles->vertexCount = wTriAmount;
		}
	...
	PushOrthogonalTransform(&rg, (float)imageWidth, (float)imageHeight);
